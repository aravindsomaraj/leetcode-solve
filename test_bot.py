        ...
        class FakeHTTP:
            def json(self, url, headers, body, **kwargs):
                ...
                return {"status": "completed", "output": [{"type": "message", "content": [
                    {"type": "output_text", "text": "class Solution {};"}]}]}
        cfg = {"model": "test-model", "reasoning_effort": "high", "max_output_tokens": 16000,
               "openai_api_key": "test-key", "leetcode_session": "PRIVATE_COOKIE"}
        http = FakeHTTP()
        bot.Solver(cfg, http).solve(problem(), "", {})
        self.assertNotIn("PRIVATE_COOKIE", json.dumps(http.request))
        self.assertEqual(http.request[2]["store"], False)
